...
func TestEncrypt(t *testing.T) {
	...
	decodestring, _ := ihash.Base64Decode(cipherstring)
	log.Println("DecryptCFB", iaes.DecryptCFB(decodestring).ToString())

	// cbc
	cipherstring2 := iaes.EncryptCBC(msg, PaddingPkcs7)
	log.Println("EncryptCBC", cipherstring2.ToStringBase64())
	decodestring2, _ := ihash.Base64Decode(cipherstring2.ToStringBase64())
	log.Println("DecryptCBC", iaes.DecryptCBC(decodestring2, cipherstring2.CurrentCipher, PaddingPkcs7).ToString())
}
...
func TestJsDecrypt(t *testing.T) {
	...
	dd := iaes.DecryptCBC(cipherText, []byte(crypted), PaddingPkcs7).ToString()
