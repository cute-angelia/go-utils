...
type (
	...
	cond struct {
		...
		// Val 查询条件对应的值
		Val interface{}
		// NotZero 不为0模式，查询条件：如：request 过来，接口得到空值，大概率会赋值为 uid = 0 ，这时设置 true， 可过滤该 Cond
		NotZero bool
	}
...
// NewCond 创建一个查询条件
func NewCond(op string, val interface{}, notZero bool) cond {
	return cond{
		Op:      op,
		Val:     val,
		NotZero: notZero,
...
//func assertCond(condIn interface{}) cond {
//	// 先尝试断言为基本类型
//	switch v := condIn.(type) {
//	case cond:
//		return v
//	case string:
//		return Cond{"=", v}
