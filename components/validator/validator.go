...
var isUrlRegexMatcher *regexp.Regexp = regexp.MustCompile(`^((ftp|iweb|https?):\/\/)?(\S+(:\S*)?@)?((([1-9]\d?|1\d\d|2[01]\d|22[0-3])(\.(1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.([0-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(([a-zA-Z0-9]+([-\.][a-zA-Z0-9]+)*)|((www\.)?))?(([a-z\x{00a1}-\x{ffff}0-9]+-?-?)*[a-z\x{00a1}-\x{ffff}0-9]+)(?:\.([a-z\x{00a1}-\x{ffff}]{2,}))?))(:(\d{1,5}))?((\/|\?|#)[^\s]*)?$`)
...
// IsStrongPassword check if the string is strong ipassword, if len(ipassword) is less than the length param, return false
// Strong ipassword: alpha(lower+upper) + number + special chars(!@#$%^&*()?><)
...
// IsWeakPassword check if the string is weak ipassword
// Weak ipassword: only letter or only number or letter + number
